...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const currentUser = await prisma.user.findFirst({ where: { email: userId } })
  if (!currentUser) return NextResponse.json({ error: 'User not found' }, { status: 404 })

  // Must be admin
  const myMembership = await prisma.projectMember.findFirst({
    where: { project_id: params.id, user_id: currentUser.id, role: 'admin' },
  })
  ...
  const existing = await prisma.projectMember.findFirst({
    where: { project_id: params.id, user_id: invitedUser.id },
  })
